...
function renderFinancials(company) {
  ...
  const riskRows = [
    ...
    { label: 'Credit Rating (S&P)',        key: 'credit_rating', fmt: v => v || '—', tip: 'S&P credit rating — a measure of creditworthiness and default risk', exportOnly: true },
...
function buildFinTable(fin, yrs, rows) {
  return '<div class="table-responsive"><table class="table ty-table mb-0">' +
    ...
    '<tbody>' +
    rows.filter(r => !r.exportOnly).map(r => {
