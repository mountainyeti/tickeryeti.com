...
function renderFinancials(company) {
  ...
  const riskRows = [
    ...
    { label: 'Earnings Per Share',        key: 'eps',      fmt: v => v != null ? '$' + v.toFixed(2) : '—', tip: 'Net income divided by diluted shares outstanding' },
    { label: 'Total Assets',              key: 'assets',   fmt: fmtBn,  tip: 'Everything the company owns or controls with economic value' },
    ...
    { label: 'Goodwill % of Total Assets', key: 'gw_pct', fmt: v => v != null ? v.toFixed(1) + '%' : '—', tip: 'Goodwill as a share of total assets — elevated values may signal acquisition risk' },
  ];

  const addlRows = [
    { label: 'EBITDA',                        key: 'ebitda',     fmt: fmtBn, tip: 'Earnings Before Interest, Taxes, Depreciation & Amortization' },
    { label: 'Total Debt (Short + Long Term)', key: 'total_debt', fmt: fmtBn, tip: 'Combined short-term and long-term borrowings' },
