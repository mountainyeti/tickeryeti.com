...
def test_search_form_visible(page: Page):
    ...
    expect(page.locator('button.ty-btn-yeti')).to_be_visible()
...
def test_financial_tabs_switch(page: Page):
    ...
    page.click('button.ty-btn-yeti')
    expect(page.locator('#ty-content')).to_be_visible(timeout=30_000)
    page.click('.ty-tab:has-text("Balance Sheet")')
    expect(page.locator('#ty-tab-balance')).to_be_visible()
    expect(page.locator('#ty-tab-income')).to_be_hidden()
